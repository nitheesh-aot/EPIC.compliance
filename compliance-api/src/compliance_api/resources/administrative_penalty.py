...
from compliance_api.schemas.administrative_penalty import (
    AdministrativePenaltyCreateSchema,
    AdministrativePenaltySchema,
...
class AdministrativePenalties(Resource):
    ...
    @staticmethod
    @auth.require
    @API.response(code=200, description="Success", model=[administrative_penalty_list_model])
    @ApiHelper.swagger_decorators(API, endpoint_description="Fetch all administrative penalties")
    @API.doc(
        params={
            "inspection_id": {
    ...
    def get():
        ...
        return administrative_penalty_list_schema.dump(administrative_penalties), HTTPStatus.OK

    @staticmethod
    @auth.require
    @ApiHelper.swagger_decorators(API, endpoint_description="Create an administrative penalty")
    @API.expect(administrative_penalty_create_model)
    @API.response(
        code=201, model=administrative_penalty_list_model, description="AdministrativePenaltyCreated"
    )
    @API.response(400, "Bad Request")
    def post():
        """Create an administrative penalty."""
        administrative_penalty_data = AdministrativePenaltyCreateSchema().load(API.payload)
        created_administrative_penalty = AdministrativePenaltyService.create_administrative_penalty(
            administrative_penalty_data
        )
        return AdministrativePenaltySchema().dump(created_administrative_penalty), HTTPStatus.CREATED


@cors_preflight("GET, OPTIONS, PATCH, DELETE")
@API.route("/<int:administrative_penalty_id>", methods=["GET", "PATCH", "DELETE", "OPTIONS"])
class AdministrativePenaltyById(Resource):
    ...
    @auth.require
    @API.response(code=200, description="Success", model=administrative_penalty_list_model)
    @ApiHelper.swagger_decorators(API, endpoint_description="Fetch an administrative penalty by id")
    def get(administrative_penalty_id):
        """Fetch an administrative penalty by id."""
        administrative_penalty = AdministrativePenaltyService.get_by_id(administrative_penalty_id)
        return AdministrativePenaltySchema().dump(administrative_penalty), HTTPStatus.OK

    @staticmethod
    @auth.require
    @ApiHelper.swagger_decorators(API, endpoint_description="Update an administrative penalty")
    @API.expect(administrative_penalty_update_model)
    @API.response(
        code=200, model=administrative_penalty_list_model, description="AdministrativePenaltyUpdated"
    )
    @API.response(400, "Bad Request")
    def patch(administrative_penalty_id):
        """Update an administrative penalty."""
        administrative_penalty_data = AdministrativePenaltyUpdateSchema().load(API.payload)
        updated_administrative_penalty = AdministrativePenaltyService.update_administrative_penalty(
            administrative_penalty_id, administrative_penalty_data
        )
        return AdministrativePenaltySchema().dump(updated_administrative_penalty), HTTPStatus.OK

    @staticmethod
    @auth.require
    @ApiHelper.swagger_decorators(API, endpoint_description="Delete an administrative penalty")
    @API.response(code=204, description="NoContent")
    def delete(administrative_penalty_id):
        """Delete an administrative penalty."""
        return AdministrativePenaltyService.delete_administrative_penalty(administrative_penalty_id)


@cors_preflight("GET, OPTIONS")
@API.route("/by-number/<string:administrative_penalty_number>", methods=["GET", "OPTIONS"])
