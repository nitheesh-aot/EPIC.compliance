...
from compliance_api.models.case_file import CaseFileStatusEnum, CaseFile as CasefileModel
from tests.utilities.factory_scenario import StaffScenario
from tests.utilities.factory_scenario import CasefileScenario
...
def test_create_case_file_with_file_number(client, auth_header):
    ...
    result = client.post(
        url, data=json.dumps(case_file_data), headers=auth_header
    )
    ...
    assert officers[0].officer_id == new_user.id
