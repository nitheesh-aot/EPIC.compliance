...
interface ShowOnlyMyInspectionsSwitchProps {
  disabled?: boolean;
...
const ShowOnlyMyInspectionsSwitch: React.FC<ShowOnlyMyInspectionsSwitchProps> = ({
  disabled = false,
  staffUsers,
  onFiltersChange,
  initialChecked = true,
  ...
  const switchLabel = useMemo(() => {
    return `${currentUser?.profile?.given_name + "'s" || "My"} Inspections`;
  ...
  const generateColumnFilters = useCallback(
    (isChecked: boolean): MRT_TableState<Inspection>["columnFilters"] => {
...
export default ShowOnlyMyInspectionsSwitch;
