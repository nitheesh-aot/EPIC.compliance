...
const TestComponent = () => {
  ...
  return (
    <div>
      ...
      <button
        data-cy="open-modal"
        onClick={() => setOpen(<div data-cy="test-modal-content">Test Modal Content</div>)}
      >
        Open Modal
      </button>
...
describe('useModal Store', () => {
  ...
  it('should open the modal and display content when setOpen is called', () => {
    ...
    cy.get('[data-cy="test-modal-content"]').should('contain', 'Test Modal Content');
  });
  ...
  it('should not open the modal if no content is provided', () => {
    act(() => {
      useModal.getState().setOpen(null);
    });
